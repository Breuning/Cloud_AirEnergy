...
static void _addStateToJson(cJSON *root, const char *name, uint8_t state);
static void _addFloatToJson(cJSON *js_list, const char *name, float value);
static void _add2FloatToJson(cJSON *root, const char *name, float value1, float value2);
...
//	_addMd5ToJson(root, "apptoken");                     //MD5不传
	...
	_addStateToJson(root, "pumpState", RS485Data.PumpState);   // 1011 代表 IO控制器IO口4321 的状态

	_addFloatToJson(root, "outTemp", RS485Data.OutWaterTemp);
	_addFloatToJson(root, "retTemp", RS485Data.RetWaterTemp);

	_add2FloatToJson(root, "eleMeter", RS485Data.Electricity1, RS485Data.Electricity2);

}

static void _addStateToJson(cJSON *root, const char *name, uint8_t state)
{
	uint8_t state_buf[10] ={0};

	itoa(state, (char *)state_buf, 2);

	cJSON_AddStringToObject(root, name, (const char*)state_buf);
}
...
static void _add2FloatToJson(cJSON *root, const char *name, float value1, float value2)
{
	...
	uint8_t value1_buf[8] ={0};
	uint8_t value2_buf[8] ={0};

	char *separator = "|";

	sprintf((char*)value1_buf, "%3.1f", value1);
	sprintf((char*)value2_buf, "%3.1f", value2);

	strcpy((char *)value_buf, (const char *)value1_buf);
	strcat((char *)value_buf, (const char *)separator);
	strcat((char *)value_buf, (const char *)value2_buf);
